...
(async () => {
  if (
    typeof route.query.begin != "string" ||
...
function exportToPDF() {
  ...
  const begin_fmt = fmt
    .format(dates.value[0])
    .replace(":", "h")
    .replace(/ /g, "")
    .replace(",", "_");
  const end_fmt = fmt
    .format(dates.value[1])
    .replace(":", "h")
    .replace(/ /g, "")
    .replace(",", "_");
  const opt = {
...
<template>
  <div v-if="dates != null" class="container">
    ...
    <div id="report-container">
      ...
      <DataTable v-if="report" :value="report" class="report-table">
        <Column
          :field="(e: ReportItem) => e.item_name"
          header="Article"
        ></Column>
        <Column :field="(e: ReportItem) => e.quantity" header="Qtt"></Column>
        <Column
          :field="(e: ReportItem) => `${e.tva * 100}%`"
          header="TVA"
        ></Column>
        <Column
          :field="(e: ReportItem) => f_price(e.subtotal_ht)"
          header="Prix HT"
        ></Column>
        <Column
          :field="(e: ReportItem) => f_price(e.subtotal_ttc)"
          header="Prix TTC"
        ></Column>
        <ColumnGroup type="footer">
          <Row>
            ...
            <Column
              :footer="
                f_price(
                  report.map((e) => e.subtotal_ht).reduce((a, b) => a + b, 0),
                )
              "
            />
            <Column
              :footer="
                f_price(
                  report.map((e) => e.subtotal_ttc).reduce((a, b) => a + b, 0),
                )
              "
            />
          </Row>
      ...
      <p v-if="report && report.length == 0" class="no-order">
        Aucune commande trouvée durant cette période !
      </p>
    </div>
    <Button
      label="Télécharger en PDF"
      :disabled="!report"
      @click="report ? exportToPDF() : {}"
      class="download-pdf"
      size="large"
      icon="pi pi-file-pdf"
    ></Button>
