...
import {
    ...
    add_variation,
    get_all_products,
} from '@/scripts/api/admin/stock/product-management'
...
async function saveEditProd(new_prod: Product) {
    ...
    if (
        await edit_product(new_prod.id, {
...
async function saveEditVariation(new_variation: Variation) {
    ...
    if (
        await edit_variation(new_variation.id, {
...
<template>
    <Dialog
        :visible="editing_product != null"
        modal
        :header="
            editing_product?.id
                ? `Produit: ${editing_product.name}`
                : 'Ajout d\'un produit'
        "
        :draggable="false"
        :closable="false"
    >
        <EditProduct
            @close="editing_product = null"
            @save="(p) => (p.id ? saveEditProd(p) : createProduct(p))"
            v-if="editing_product != null"
            :product="editing_product"
        />
    </Dialog>
    <Dialog
        :visible="editing_variation != null"
        modal
        :header="
            editing_variation?.id
                ? `Variation: ${editing_variation.name}`
    ...
    >
        <EditVariation
            @close="editing_variation = null"
            @save="(v) => (v.id ? saveEditVariation(v) : createVariation(v))"
            v-if="editing_variation"
            :variation="editing_variation"
        />
    </Dialog>
    ...
    <ProductViewAdmin
        ...
        @request-edit-product="(prod) => (editing_product = prod)"
        @refresh_stock="refresh_stock"
        @request-add-variation="requestCreateVariation"
        @request-edit-variation="(variation) => (editing_variation = variation)"
    />
</template>
