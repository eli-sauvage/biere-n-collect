...
async function toogleVariationAvailable(variation: Variation) {
    if (
        await edit_variation(variation.id, {
            new_available_to_order: !variation.available_to_order,
        })
    ) {
        emit('refresh_stock')
    } else {
        ...
        <div class="bottom">
            ...
            <div v-for="variation in product.variations" class="variation">
                <div class="variation-details">
                    ...
                    <ToggleSwitch
                        :modelValue="variation.available_to_order"
                ...
                <div class="edit-variation">
                    <Button
                        icon="pi pi-pencil"
                        severity="primary"
                        @click="$emit('requestEditVariation', variation)"
                        :badge="f_price(variation.price_ttc)"
                    ></Button>
                    <Button
                        icon="pi pi-trash"
                        severity="danger"
                        @click="(e) => confirmDeleteVariation(e, variation)"
                    ></Button>
                </div>
            </div>
            <Button
                ...
                icon="pi pi-plus"
                @click="$emit('requestAddVariation', product)"
            ></Button>
            <Divider />
            <div class="edit-product">
                <Button
                    icon="pi pi-pencil"
                    severity="primary"
                    @click="$emit('requestEditProduct', product)"
                ></Button>
                <Button
                    icon="pi pi-trash"
                    severity="danger"
                    @click="comfirmDeleteProduct"
                ></Button>
            </div>
        </div>
    </div>
