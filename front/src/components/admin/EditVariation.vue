...
function changePriceHT(new_price_ht: InputNumberInputEvent) {
    if (typeof new_price_ht.value != 'number') return
    price_ht.value = new_price_ht.value
    price_ttc.value = price_ht.value * (1 + newVariation.value.tva)
}

function changeTVA(new_tva: InputNumberInputEvent) {
    if (typeof new_tva.value != 'number') return
    newVariation.value.tva = new_tva.value / 100
    price_ttc.value = price_ht.value * (1 + newVariation.value.tva)
}

function changePriceTTC(new_price_ttc: InputNumberInputEvent) {
    if (typeof new_price_ttc.value != 'number') return
    ...
    <form>
        <div class="inputs">
            ...
            <div class="available">
                ...
                <ToggleSwitch
                    id="available"
                    v-model="newVariation.available_to_order"
                />
            </div>
        </div>
        <div class="footer">
