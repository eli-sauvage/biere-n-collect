...
let variations_with_cart_elem: Ref<[Variation, CartElement][]> = ref(
    props.product.variations.map((variation) => [
        variation,
        props.cart.getElement(variation.id),
    ])
)
...
<template>
    <div class="prod">
        ...
        <div
            v-for="([variation, cartElem], index) in variations_with_cart_elem"
        >
            ...
            <div class="variation">
                ...
                <div class="add-and-price">
                    <InputNumber
                        v-if="cartElem.quantity > 0"
                        :model-value="cartElem.quantity"
                        ...
                        @update:model-value="
                            (e) => {
                                cartElem.setQuantity(0, 0)
                                cartElem.setQuantity(
                                    e,
                                    remaining_quantity() / variation.volume
                                )
                            }
                        "
                    >
                    ...
                    <Button
                        ...
                        class="add-to-cart"
                        @click="(e) => addOne(e, cartElem)"
                        :disabled="remaining_quantity() < variation.volume"
                        :badge="f_price(variation.price_ttc)"
                    ></Button>
                </div>
            </div>
