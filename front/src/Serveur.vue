...
const toggle_served = async () => {
    if (selected_order.value == null) return
    let res = await set_served(
        selected_order.value,
        !selected_order.value?.served
    )
    if (!res) return
    let order = await get_order_by_id(selected_order.value.id)
    if (order) selected_order.value = order
}
...
<template>
    ...
    <div class="container">
        ...
        <div
            ...
            :style="`display: ${isScanning ? 'unset' : 'none'}`"
        >
        ...
        <Panel v-if="!isScanning">
            <form class="recherche">
                ...
                <label for="receipt-search-order"
                    >Chercher par n° de reçu</label
                >
    ...
    <Dialog
        ...
        :visible="selected_order != null"
        @after-hide="console.log"
        :closable="false"
        v-if="selected_order != null"
    >
        <DisplayOrder
            :selected_order="selected_order"
            @served_clicked="toggle_served"
        />
        <DataTable :value="selected_order.detail">
            ...
            <ColumnGroup type="footer">
                <Row>
                    ...
                    <Column
                        v-if="selected_order != null"
                        :footer="f_price(selected_order.total_price_ttc)"
                    />
                </Row>
            </ColumnGroup>
        </DataTable>
        <div class="close-selected-order-btn">
            <Button
                ...
                @click="setServed(selected_order, false)"
                severity="secondary"
            ></Button>
            <Button
                v-if="!selected_order.served"
