    ...
    if (res.error) {
      ...
      for (let i = 0; i < prods.length; i++) {
        for (let j = 0; j < prods[i].variations.length; j++) {
          prods[i].variations[j].price_ttc =
            prods[i].variations[j].price_ht * (1 + prods[i].variations[j].tva);
        }
      }
      return prods;
    }
...
export async function edit_product(
  ...
  let url =
    `${base}/admin/stock/products?product_id=${encodeURIComponent(product_id)}` +
...
export async function move_product(
  product_id: number,
  direction: "up" | "down",
): Promise<boolean> {
...
export async function add_variation(
  product_id: number,
  name: string,
  price_ht: number,
  tva: number,
  ...
  let url =
    `${base}/admin/stock/products/add_variation?product_id=${encodeURIComponent(product_id)}` +
...
export async function remove_variation(
  product_id: number,
  variation_id: number,
): Promise<boolean> {
  let url =
    `${base}/admin/stock/products/remove_variation?product_id=${encodeURIComponent(product_id)}` +
