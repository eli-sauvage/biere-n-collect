...
export async function create_challenge(email: string): Promise<boolean> {
    ...
    let error_title = "Erreur lors de la création du challenge d'authentification"
    try {
...
export async function verify_challenge(email: string, code: string): Promise<boolean> {
